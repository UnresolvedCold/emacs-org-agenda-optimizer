...
public class AgendaTodoProvider implements TodoProvider {
  ...
  /***
   * If an item has a scheduled time then return true else return false
   * @param todo
   * @return true if item is pinned else false
   */
  private static boolean isPinned(Todo todo) {
    boolean isPinned = todo.getScheduledTime() != null;
      return isPinned;
  }
  ...
  public List<TodoItem> getTodos() {
    List<Todo> todos = getTodosFromFilesAndDirectories();

    return convertPojoTodoToApiTodoItem(todos);
  }

  private List<TodoItem> convertPojoTodoToApiTodoItem(List<Todo> todos) {
    ...
    for (Todo todo : todos) {
      ...
      TodoItem todoItem =
          new TodoItem(todo.getTitle(), todo.getEffort(), itemType, isPinned, priority);
  ...
  private List<Todo> getTodosFromFilesAndDirectories() {
  ...
  /***
   * If an item has a priority then return it else return MEDIUM
   * @param todo
   * @return Priority of the item
   */
  private Priority getPriority(Todo todo) {
  ...
  /***
   * Get all the TODO items from the org directory
   * @param dir
   * @return
   */
  ...
  /***
   * Get all the TODO items from the org file
   * @param filename
   * @return
   */
  ...
  private void computeChildTagAsPerParent(
      OrgNodeInList head, Map<Integer, List<String>> levelTags, Integer level) {
    if (level == 1) {
      levelTags.clear();
    }
