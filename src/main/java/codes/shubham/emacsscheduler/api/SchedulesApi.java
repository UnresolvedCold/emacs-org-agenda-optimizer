...
public class SchedulesApi {
  @Autowired AgendaTodoProvider agendaTodoProvider;

  @Autowired private SolverManager<Schedule, String> solverManager; // schedule and id of item
  ...
  public List<SchedulesOutput> schedule() throws ExecutionException, InterruptedException {
    ...
    List<TodoItem> tasksToSchedule = agendaTodoProvider.getTodos();
    ...
    for (TodoItem todoItem : schedule.getTodoItems()) {
      ...
      out.setDuration(todoItem.getDuration());
  ...
  private List<DateTime> divideDayInTimeBlocks(int totalTaskDuration) {
    ...
    currentTime =
        currentTime.plusMinutes(
            idealDayTimeBlockDuration
                - (currentTime.getMinuteOfHour() % idealDayTimeBlockDuration));
